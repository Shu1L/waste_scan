...
class mysqlmanager:
    ...
    def t_report_vuln(self, result, high_count, middle_count, low_count, notice_count):
        if self.t_task_exist():
            try:
                self._client.root.client_update_reports(self._taskid, result, high_count, middle_count, low_count,
                                                        notice_count)
            except Exception as e:
                log.error(str(e))
        else:
            ...
            try:
                self._client.root.client_insert_reports(self._taskid, self._website, result, high_count, middle_count,
                                                        low_count, notice_count)
            except Exception as e:
                log.error(str(e))
